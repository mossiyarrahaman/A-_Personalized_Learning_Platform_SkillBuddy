...
const TopicDetailModal = ({ courseId, moduleId, topicId, onClose, onUpdate }) => {
    ...
    const [quizStep, setQuizStep] = useState('setup'); // setup, loading, quiz, submitting, result
    const [bloomLevel, setBloomLevel] = useState('understand');
    const [quizData, setQuizData] = useState(null);
    const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
    const [selectedAnswer, setSelectedAnswer] = useState(null);
    const scoreRef = useRef(0); // Use ref to avoid stale closure in submitQuiz
    ...
    const generateQuiz = async () => {
        ...
        try {
            ...
            if (courseId && courseId !== 'undefined') {
                // ── Teacher course: use course-based quiz endpoint ──
                res = await api.get(
                    `/courses/${courseId}/module/${moduleId}/topic/${topicId}/quiz?bloomLevel=${bloomLevel}`
                );
            } else {
                // ── AI learning path: use assessment context endpoint ──
                res = await api.post('/assessments/generate-from-context', {
                    courseId: null,
                    moduleId,
                    topicId,
                    bloomLevel
            ...
            // Normalise response shape — both endpoints return differently
            ...
            if (questions && questions.length > 0) {
                ...
                setQuizError('No questions were returned. Please try again.');
                setQuizStep('setup');
            }
        } catch (error) {
    ...
    const submitQuiz = async () => {
        ...
        try {
            ...
            if (courseId && courseId !== 'undefined') {
                // Submit to course progress endpoint
                const res = await api.post(
                    `/courses/${courseId}/module/${moduleId}/topic/${topicId}/quiz/submit`,
                    {
                ...
                // AI path — no backend submit needed, just show result locally
                const passed = finalScorePercentage >= 70;
                setQuizResult({
                    passed,
                    score: finalScorePercentage,
                    topicCompleted: passed
                });
    ...
    const fetchTopicDetails = async () => {
        try {
            setLoading(true);
            const queryParams = (courseId && courseId !== 'undefined') ? `?courseId=${courseId}` : '';
                ...
                <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-[#0a0a0a]">
                    {loading ? (
                        ...
                        <div className="space-y-10">
                                ...
                                <div className="space-y-4">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center gap-3 mt-1 text-sm text-gray-500">
                                                    <span className="capitalize px-2 py-0.5 rounded bg-gray-800">
                                                        {resource.type === 'article' ? 'Document' : resource.type}
                                                    </span>
                                                    <span>•</span>
                                    ...
                                    {(!topic?.resources || topic.resources.length === 0) && (
                                        <div className="text-gray-500 text-center py-4 italic">
                                            No external links found for this topic.
                                        </div>
                                    )}
                                </div>
                            ...
                            {/* Quiz Section — available for BOTH teacher courses and AI path */}
                                ...
                                previewFile.url?.includes('localhost') || previewFile.url?.includes('127.0.0.1') ? (
                                    <div className="text-center space-y-4">
                                        ...
                                        <p className="text-gray-400 max-w-md mx-auto">
                                            Microsoft/Google Viewers cannot preview files hosted on localhost. Please download to view.
                                        </p>
                                    </div>
                                ) : (
                                    <iframe
                    ...
                    <div className="bg-gray-900 w-full max-w-2xl rounded-2xl border border-purple-500/30 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                        ...
                        <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
                            ...
                            {quizStep === 'loading' && (
                                <div className="flex flex-col items-center justify-center py-12 text-center h-64">
                                    ...
                                    <p className="text-gray-400 mt-2">
                                        AI is crafting questions at the{' '}
                                        <span className="text-purple-400 capitalize">{bloomLevel}</span> level.
                                    </p>
                                </div>
                            )}
                            ...
                            {quizStep === 'submitting' && (
                                <div className="flex flex-col items-center justify-center py-12 text-center h-64">
                                    <Loader className="w-12 h-12 text-purple-500 animate-spin mb-4" />
                                    <h3 className="text-xl font-bold text-white">Saving your results...</h3>
                                </div>
                            )}
                                    ...
                                    {showExplanation && (() => {
                                        ...
                                        return (
                                            <div className={`mt-4 border p-4 rounded-xl flex gap-3 animate-fade-in ${isCorrect ? 'bg-green-900/20 border-green-500/30' : 'bg-red-900/10 border-red-500/30'}`}>
                                                ...
                                                <div>
                                                    <h4 className={`font-bold mb-1 ${isCorrect ? 'text-green-100' : 'text-red-100'}`}>
                                ...
                                <div className="text-center py-8 space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-gray-400 text-lg">
                                            You scored{' '}
                                            <span className={`font-bold ${quizResult.passed ? 'text-green-400' : 'text-red-400'}`}>
                                                {quizResult.score}%
                                            </span>
                                            {' '}({scoreRef.current}/{quizData?.length} correct)
                                        </p>
                                        {!quizResult.passed && (
                                            <p className="text-sm text-gray-500 mt-2">You need 70% to complete this topic.</p>
                                        )}
                                    </div>
                                    <div className="flex gap-3 justify-center mt-8">
                                        <button
                                            onClick={() => { setQuizStep('setup'); scoreRef.current = 0; setScoreDisplay(0); setQuizError(null); }}
